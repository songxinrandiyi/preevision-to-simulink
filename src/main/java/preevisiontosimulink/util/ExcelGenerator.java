...
public class ExcelGenerator {
	...
	private static Sheet createSheetWithHeaders(Workbook workbook, String modelName, boolean isUpdated) {
		...
		if (isUpdated) {
			String[] headers = { "Wire", "Typical current", "Max Peak Current", "Max Peak Current Duration",
					"WireConnector1", "Pinnummer1", "WireConnector2", "Pinnummer2", "Schematic Pin", "Ø", "Wire Length",
					"Voltage Drop with 1A" };
			createHeaderCells(headerRow, headers);
		} else {
			String[] headers = { "Wire", "Component1", "WireConnector1", "Pinnummer1", "SchematicPin1", "PinType1",
					"SealType1", "Typical Current", "Max Peak Current", "Max Peak Current Duration", "Component2",
					"WireConnector2", "Pinnummer2", "Schematic Pin2", "PinType2", "SealType2", "Typical current",
					"Max Peak Current", "Max Peak Current Duration", "WireType", "Ø", "Wire Length", "FeatureNo.",
					"Feature", "GeometryDiagram" };
			createHeaderCells(headerRow, headers);
		}
	...
	private static List<SimulinkSubsystem> filterAndSortSubsystems(List<SimulinkSubsystem> subsystems) {
		...
		// Sort the subsystems by the name of the first contact point, and if the names
		// are equal, by pinNumberTo
	...
	private static void createDataRow(Row row, SimulinkSubsystem subsystem) {
		if (subsystem.getKabelInformation().getWireNumber() != null) {
			row.createCell(0).setCellValue(subsystem.getKabelInformation().getWireNumber());
		}
		...
		if (subsystem.getKabelInformation().getCrossSectionArea() != null) {
			row.createCell(20).setCellValue(subsystem.getKabelInformation().getCrossSectionArea());
			row.createCell(8).setCellValue(subsystem.getKabelInformation().getCrossSectionArea() * 6);
			row.createCell(17).setCellValue(subsystem.getKabelInformation().getCrossSectionArea() * 6);
		}
		if (subsystem.getKabelInformation().getLength() != null) {
			row.createCell(21).setCellValue(subsystem.getKabelInformation().getLength());
		}
		if (subsystem.getKabelInformation().getSignalName() != null) {
			row.createCell(4).setCellValue(subsystem.getKabelInformation().getSignalName());
			row.createCell(13).setCellValue(subsystem.getKabelInformation().getSignalName());
		}
	}
	...
	private static void createUpdatedDataRow(Row row, SimulinkSubsystem subsystem, CellStyle leftAlignStyle) {
		...
		if (subsystem.getKabelInformation().getCrossSectionArea() != null) {
			row.createCell(9).setCellValue(subsystem.getKabelInformation().getCrossSectionArea());
		}
		if (subsystem.getKabelInformation().getLength() != null) {
			row.createCell(10).setCellValue(subsystem.getKabelInformation().getLength());
		}
		if (subsystem.getKabelInformation().getCrossSectionArea() != null
				&& subsystem.getKabelInformation().getLength() != null) {
			double voltageDrop = 1 * CalculatorUtils.calculateResistance(subsystem.getKabelInformation().getLength(),
					subsystem.getKabelInformation().getCrossSectionArea());
			row.createCell(11).setCellValue(voltageDrop);
		}
		if (subsystem.getKabelInformation().getSignalName() != null) {
			row.createCell(8).setCellValue(subsystem.getKabelInformation().getSignalName());
		}
	}
