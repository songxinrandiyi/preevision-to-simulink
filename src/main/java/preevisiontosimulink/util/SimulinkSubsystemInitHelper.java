...
public class SimulinkSubsystemInitHelper {
	public static void initStecker(SimulinkSubsystem subsystem) {
		...
		if (inConnections != null) {
			for (LConnection inPort : inConnections) {
				...
				subsystem.addConnection(new SimulinkConnection(subsystem.getBlock(inPort.getName() + "_I").getOutPort(1),
						subsystem.getBlock(subsystem.getName() + "_E").getInPort(0), subsystem));
				subsystem.addConnection(new SimulinkConnection(subsystem.getBlock(inPort.getName() + "_I").getOutPort(0),
						subsystem.getBlock(inPort.getName() + "_PS").getInPort(0), subsystem));
				subsystem.addConnection(new SimulinkConnection(subsystem.getBlock(inPort.getName() + "_PS").getOutPort(0),
	...
	public static void initCabel(SimulinkSubsystem subsystem, Double resistance) {
		...
		subsystem.addConnection(new SimulinkConnection(inPort.getInPort(0), resistor.getInPort(0), subsystem));

		subsystem.addConnection(new SimulinkConnection(resistor.getOutPort(0), outPort.getInPort(0), subsystem));

		subsystem.addConnection(new SimulinkConnection(resistor.getOutPort(0), voltageSensor.getOutPort(1), subsystem));
		subsystem.addConnection(new SimulinkConnection(resistor.getInPort(0), voltageSensor.getInPort(0), subsystem));
		subsystem.addConnection(new SimulinkConnection(voltageSensor.getOutPort(0), converter.getInPort(0), subsystem));
		subsystem.addConnection(new SimulinkConnection(converter.getOutPort(0), display.getInPort(0), subsystem));
	}
	...
	public static void initThermalKabel(SimulinkSubsystem subsystem, Connection connection) {
		...
		subsystem.addConnection(new SimulinkConnection(inPort.getOutPort(0), square.getInPort(0), subsystem));
		subsystem.addConnection(new SimulinkConnection(square.getOutPort(0), resistance.getInPort(0), subsystem));
		subsystem.addConnection(new SimulinkConnection(resistance.getOutPort(0), powerDiff.getInPort(0), subsystem));
		subsystem.addConnection(new SimulinkConnection(powerDiff.getOutPort(0), thermalCapacityInverse.getInPort(0), subsystem));
		subsystem.addConnection(new SimulinkConnection(thermalCapacityInverse.getOutPort(0), integrator.getInPort(0), subsystem));
		subsystem.addConnection(new SimulinkConnection(integrator.getOutPort(0), outPort.getInPort(0), subsystem));
		
		subsystem.addConnection(new SimulinkConnection(integrator.getOutPort(0), temperatureDiff.getInPort(0), subsystem));
		subsystem.addConnection(new SimulinkConnection(environmentTemperature.getOutPort(0), 
				temperatureDiff.getInPort(1), subsystem));
		subsystem.addConnection(new SimulinkConnection(temperatureDiff.getOutPort(0), product.getInPort(0), subsystem));
		subsystem.addConnection(new SimulinkConnection(thermalResistance.getOutPort(0), inverse.getInPort(0), subsystem));
		subsystem.addConnection(new SimulinkConnection(inverse.getOutPort(0), product.getInPort(1), subsystem));
		subsystem.addConnection(new SimulinkConnection(product.getOutPort(0), powerDiff.getInPort(1), subsystem));
	}
	...
	public static void initConnection(Connection connection, SimulinkSystem system, SimulinkSubsystem subsystem,
		...
		if (system.getRelation(pathStart) == null) {
			system.addConnection(startRelation);
		...
		if (system.getRelation(pathEnd) == null) {
			system.addConnection(endRelation);
	...
	public static void generateThermalConnector(GeneralWire generalWire, SimulinkSystem system) {
		for (Connection connection : generalWire.getValidConnections()) {
			...
			subsystemStart.addConnection(new SimulinkConnection(currentSource.getOutPort(0), 
			...
			subsystemEnd.addConnection(new SimulinkConnection(inPort.getOutPort(0), 
